...
from app.models.models import timetable_slots, teachers, batches, users
...
def create_slot(
    ...
    if not (_is_admin(current_user) or _is_teacher(current_user)):
...
def get_my_slots(
    ...
    if day:
        q = q.filter(timetable_slots.day == day)
...
def update_slot(
    ...
    if not (
        ...
        or (teacher_owner and teacher_owner.user_id == current_user.id)
    ):
        raise HTTPException(403, "Not allowed")
